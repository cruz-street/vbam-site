...
- `wbraid`
- `msclkid`
- `landing_referrer` — added for organic traffic that never carries a UTM
  tag (a bare link in an Instagram/Facebook caption reads badly with a query
  string appended). Captures `document.referrer`'s origin on first landing —
  e.g. `https://m.facebook.com` — the same way GA4 already classifies this
  traffic as "facebook.com / referral." Not populated for QR/print or SMS,
  since those never have a referring webpage — use a real `utm_source` for
  those channels instead, this field only helps organic web clicks.
