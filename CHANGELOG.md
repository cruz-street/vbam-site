...
### Added
- **Staging re-homed to Jesse's CF tenant (2026-06-05).** New `vbam-dev` Pages project (production_branch=staging) replaces the Ashwin-account `vbam-site` project as the staging target; staging now serves at `dev.verobeachadultmedicine.com` (+ `vbam-dev.pages.dev`). CMS auth Worker `vbam-cms-auth` redeployed into Jesse's tenant (`vbam-cms-auth.jesse-037.workers.dev`); Decap `config.yml` `base_url`/`logo_url` updated. deploy.yml staging step retargeted (project + account ID + token). Old project abandoned in place — never deploy to it. Rule 15 widened to any cruzstreet.com identity. _Pending: Worker secrets (GitHub OAuth client ID/secret, ALLOWED_DOMAINS), `dev` CNAME record._
